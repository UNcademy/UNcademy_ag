...
const resolvers$1 = {
    Query: {
        searchByProfessor: (_, { teacherName }) => {
            return generalRequest(`${URL$1}/groupByProfessor/${teacherName}`, 'GET')
        }
    }
};
...
const combinedResolvers = {
    Mutation: {
        createClassListAuto: async (_, {teacherName}) => {
            const data = await resolvers$1.Query.searchByProfessor(_,{teacherName});
            const classList = {
            ...
            return resolvers.Mutation.createClassList(_, {classList})
        }
    }
};
