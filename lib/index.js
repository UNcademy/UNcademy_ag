...
const registrationResolvers = {
    ...
    Mutation: {
        createRegistration: (_, {registration}) => {
            console.log(registration.getRegistration);
...
const port$3 = '3333';
const entryPoint$2 = 'academic-record';

const URL$3 = `http://${url$3}:${port$3}`;

const requests$3 = {
    getAcademic:(_,{id})=>
        generalRequest(`${URL$3}/${entryPoint$2}/${id}`, 'GET')  ,

    getAllAcademic:(_)=>
        generalRequest(`${URL$3}/${entryPoint$2}`, 'GET'), 
    
    getMaterias:(_,{id})=>
        generalRequest(`${URL$3}/${entryPoint$2}/materias/${id}`, 'GET')  ,
    
    getPrograma:(_)=>
        generalRequest(`${URL$3}/${entryPoint$2}/student-program/${id}`, 'GET'),

    createAcademic:(_,{academic})=>
        generalRequest(`${URL$3}/${entryPoint$2}`, 'POST', academic),
...
const academicResolvers = {
    Query: {
        getAcademic:(_,{id})=>{
            return requests$3.getAcademic(_,{id})
        },
        getAllAcademic:(_)=>{
            return requests$3.getAllAcademic(_)
        },

        getMaterias:(_,{id})=>{
            return requests$3.getMaterias(_,{id})
        },
        getPrograma:(_,{id})=>{
            return requests$3.getPrograma(_,{id})
        }
    ...
    Mutation: {
        createAcademic:(_, {academic})=>{
            return requests$3.createAcademic(_,{academic})
